...
from benchmarking.evaluation_models import stumpy_matching, dtaidistance_matching

def process_retrieval_results(episode_results, top_k=10, max_distance=None):
...
def stumpy_evaluation(output_path, stumpy=True, dtaidistance=False):
    with h5py.File(output_path, 'w') as outfile:
        ...
        with h5py.File(target_data_path, 'r') as f:
            for episode_key in f.keys():
                ...
                episode_results = []
                for offline_file in offline_file_list:
                    with h5py.File(offline_file, 'r') as offline_f:
                        offline_data = offline_f['data']
                        for demo_key in list(offline_data.keys())[:10]:  # Limit to first 10 demos for speed
                ...
                processed_results = process_retrieval_results(episode_results, top_k=5)
...
if __name__ == "__main__":
    target_data_path = "data/libero_retrieval/task_datasets/libero_10/libero_10.hdf5"
    offline_data_dir = "data/libero_retrieval/offline_datasets/libero_90/"
    offline_file_list = glob.glob(os.path.join(offline_data_dir, "*.hdf5"))
    
    stumpy_evaluation("data/retrieval_results/retrieval_results_stumpy.hdf5", stumpy=True, dtaidistance=False)

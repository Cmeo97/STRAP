...
from plotnine import (
    ...
    scale_fill_gradient2, scale_fill_gradientn, scale_fill_discrete,
    scale_color_discrete, facet_wrap, facet_grid, ggsave,
    element_text, element_blank, element_rect, element_line,
    guides, guide_colorbar, guide_legend, coord_flip,
...
def parse_filename(filename: str) -> Tuple[str, str]:
    """
    Parse filename to extract dataset and method.
    Format: [dataset]_retrieval_evaluation_[method].json
    """
...
def find_all_evaluation_files(results_dir: str) -> List[str]:
    ...
    pattern = os.path.join(results_dir, "*_retrieval_evaluation_*.json")
...
def plot_episode_metric_heatmap(df: pd.DataFrame, out_dir: str):
    """Episode × Metric heatmap (z-scored)."""
    z = (df - df.mean()) / (df.std() + 1e-8)
    
    # Convert to long format for plotnine
    z_df = z.reset_index().melt(id_vars='episode', var_name='metric', value_name='z_score')
    
    p = (ggplot(z_df, aes(x='metric', y='episode', fill='z_score'))
         + geom_tile(color='white', size=0.5)
         + scale_fill_gradient2(low='#053061', mid='white', high='#67001f', 
                               midpoint=0, limits=(-2, 2), name='z-score')
         + labs(x='Metric', y='Episode', title='Episode × Metric heatmap (z-scored)')
         + theme_minimal()
         + theme(
             axis_text_x=element_text(rotation=45, hjust=1),
             plot_title=element_text(size=14, weight='bold'),
             axis_title=element_text(size=12),
             aspect_ratio=1.0/len(df) * len(df.columns) if len(df) > 0 else 1
         )
    )
    height = max(6, 0.3 * len(df))
    _save(p, os.path.join(out_dir, "episode_metric_heatmap.png"), width=10, height=height)
...
def load_all_metrics(results_dir: str) -> pd.DataFrame:
    ...
    Returns DataFrame with columns: dataset, method, episode, and all metrics.
    """
    ...
    for filepath in files:
        dataset, method = parse_filename(filepath)
...
def plot_ranking_comparison(combined_df: pd.DataFrame, out_dir: str):
    """Rank methods by each metric per dataset (lower is better for distance metrics, higher for others)."""
    metric_cols = [c for c in combined_df.columns if c not in ['episode', 'dataset', 'method']]
    
    # Metrics where lower is better
    lower_better = ['wasserstein', 'dtw_nn', 'spectral_wasserstein']
    
    # Compute mean per dataset-method
    means = combined_df.groupby(['dataset', 'method'])[metric_cols].mean().reset_index()
    ...
    for dataset in datasets:
        ...
        _save(p, os.path.join(out_dir, filename), width=width, height=height)


def plot_metric_multibar_per_dataset(combined_df: pd.DataFrame, out_dir: str):
    """
    ...
    # Compute mean per dataset-method combination
    means = combined_df.groupby(['dataset', 'method'])[metric_cols].mean().reset_index()
    ...
    for metric in metric_cols:
        ...
        _save(p, os.path.join(out_dir, filename), 
...
def visualize(metrics_path: str, output_dir: Optional[str] = None, 
    ...
    try:
        ...
        # Method rankings (only non-heatmap comparison visualization)
        plot_ranking_comparison(combined_df, output_dir)
        
        # Multi-bar graphs for each metric per dataset

...
class UnsupervisedRetrievalEvaluator:
    ...
    def distributional_coverage(self, k: int = 5) -> Dict[str, float]:
        """
        Precision / recall style coverage in flattened feature space.

        Uses k-NN radius per reference sample.
        """
        print("Running distributional_coverage metric...")
        r = self._flatten(self.retrieved)
        ref = self._flatten(self.reference)

        d_ref = cdist(ref, ref)
        np.fill_diagonal(d_ref, np.inf)

        # tqdm less useful here, but add for radii loop
        radii = np.partition(d_ref, k, axis=1)[:, k]

        d_cross = cdist(ref, r)

        precision = (d_cross.min(axis=0) <= radii.mean()).mean()
        recall = (d_cross.min(axis=1) <= radii).mean()
        coverage = (d_cross.min(axis=1) <= radii).mean()
        density = (d_cross <= radii[:, None]).sum(axis=0).mean() / k
        ...
        print(f"Finished distributional_coverage: {result}")
        return result

...
def dtw_distance_torch(proto: torch.Tensor, seq: torch.Tensor) -> torch.Tensor:
    """
    Straight-through DP-DTW:
    - forward: true DTW cost
    - backward: surrogate framewise distance
    """

    # Pairwise distances (grad-enabled)
    dist = torch.cdist(proto, seq, p=2) ** 2  # (Lp, Ls)

    # ----- Forward (no grad) -----
    with torch.no_grad():
        acc = compute_accumulated_cost_matrix_subsequence_dtw_21(
            dist.detach().cpu().numpy()
        )
        dtw_cost = acc[-1].min() / proto.shape[0]

    # ----- Backward surrogate -----
    surrogate = dist.mean()

    # Straight-through estimator
    return surrogate + (dtw_cost - surrogate.detach())
...
def dp_dtw_loss(
    ...
    """
    temporal_embeddings:
        (B * Samples, L_max, D)  padded
    """
    device = temporal_embeddings.device
    samples_per_episode = n_way * (k_shot + n_query)
    batch_size = temporal_embeddings.size(0) // samples_per_episode

    temporal_embeddings = temporal_embeddings.view(
        batch_size, samples_per_episode, temporal_embeddings.size(1), -1
    )

    total_loss = 0.0

    for b in range(batch_size):
        support = temporal_embeddings[b, :n_way * k_shot]
        query   = temporal_embeddings[b, n_way * k_shot:]

        # --- Build temporal prototypes ---
        prototypes = []
        for c in range(n_way):
            class_support = support[c * k_shot:(c + 1) * k_shot]

            # interpolate to median length
            lengths = [s.shape[0] for s in class_support]
            Lm = int(np.median(lengths))

            resized = []
            for s in class_support:
                s = s.unsqueeze(0).permute(0, 2, 1)
                r = torch.nn.functional.interpolate(
                    s, size=Lm, mode="linear", align_corners=True
                )
                resized.append(r.squeeze(0).permute(1, 0))

            prototypes.append(torch.stack(resized).mean(0))

        # --- DTW distances ---
        logits = []
        rep_loss = 0.0

        for qi in range(n_way * n_query):
            seq = query[qi]
            dists = []
            for c in range(n_way):
                d = dtw_distance_torch(prototypes[c], seq)
                dists.append(d)

                if qi // n_query == c:
                    rep_loss += d

            logits.append(torch.stack(dists))

        logits = torch.stack(logits)           # (Q, Way)
        labels = torch.arange(n_way).repeat_interleave(n_query).to(device)

        ce = torch.nn.functional.cross_entropy(-logits, labels)
        total_loss += ce + lambda_rep * rep_loss / (n_way * n_query)
...
if __name__ == '__main__':
    ...
    print(reference_maneuver_data[0][0].shape) # (9, 10, 96, 12)
    print(class_names)
    ...
    if not pretrained:
        ...
        torch.save(encoder.state_dict(), f'prototype_libero_model_{N_DIM}_dp_dtw.pth')

    else:
        # load the trained model (for inference)
        encoder.load_state_dict(torch.load(f'prototype_libero_model_{N_DIM}_dp_dtw.pth'))
    ...
    with open(f'prototype_results_{N_DIM}_dp_dtw.json', 'w') as f:

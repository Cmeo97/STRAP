...
class SoftDTW(nn.Module):
    """
    Differentiable Soft-DTW (Cuturi & Blondel, 2017)
    Batched implementation for (B, L1, D) vs (B, L2, D)
    """
    ...
    def forward(self, x, y):
        ...
        R = torch.full((B, Lx + 1, Ly + 1), float("inf"), device=x.device)
        R[:, 0, 0] = 0.0
...
def robust_prototypical_loss_softdtw(
    embeddings, n_way, k_shot, n_query, gamma=0.1
):
    """
    embeddings: (B * Samples, L, D)
    """
    device = embeddings.device
    samples_per_episode = n_way * (k_shot + n_query)
    batch_size = embeddings.size(0) // samples_per_episode

    embeddings = embeddings.view(batch_size, samples_per_episode, *embeddings.shape[1:])

    support = embeddings[:, :n_way * k_shot]
    query = embeddings[:, n_way * k_shot:]

    # Prototypes: average sequences (interpolate to median length)
    prototypes = []
    for b in range(batch_size):
        proto_b = []
        for c in range(n_way):
            seqs = support[b, c * k_shot:(c + 1) * k_shot]
            ...
            for s in seqs:
                ...
                s = torch.nn.functional.interpolate(
                    s, size=median_len, mode="linear", align_corners=True
                )
                resized.append(s.squeeze(0).permute(1, 0))
            proto_b.append(torch.stack(resized).mean(dim=0))
        prototypes.append(torch.stack(proto_b))
    prototypes = torch.stack(prototypes)  # (B, Way, Lp, D)

    soft_dtw = SoftDTW(gamma=gamma)
    ...
    for b in range(batch_size):
        q = query[b]  # (Q, Lq, D)
        p = prototypes[b]  # (Way, Lp, D)

        Q = q.shape[0]
        q_exp = q.unsqueeze(1).expand(Q, n_way, -1, -1)
        p_exp = p.unsqueeze(0).expand(Q, n_way, -1, -1)

        q_flat = q_exp.reshape(-1, q.shape[1], q.shape[2])
        p_flat = p_exp.reshape(-1, p.shape[1], p.shape[2])

        dists = soft_dtw(q_flat, p_flat)
        dists = dists.view(Q, n_way)

        logits = -dists
        losses.append(
            torch.nn.functional.cross_entropy(logits, target_labels)
        )
...
def train_robust_network(encoder, maneuver_data):
    ...
    for i, (batch_sequences, mask) in enumerate(dataloader):
        ...
        embeddings = encoder(batch_sequences, return_temporal=True)

        
        # 4. Compute Loss (Handles the Batch Dim automatically)
        loss = robust_prototypical_loss_softdtw(
            embeddings, N_WAY, K_SHOT, N_QUERY, gamma=0.1
        )
...
if __name__ == '__main__':
    ...
    if not pretrained:
        ...
        torch.save(encoder.state_dict(), f'prototype_libero_model_{N_DIM}_softdtw.pth')

    else:
        # load the trained model (for inference)
        encoder.load_state_dict(torch.load(f'prototype_libero_model_{N_DIM}_softdtw.pth'))
    ...
    with open(f'prototype_results_{N_DIM}_softdtw.json', 'w') as f:

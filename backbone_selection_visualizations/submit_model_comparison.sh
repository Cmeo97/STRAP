#!/bin/bash
#SBATCH --job-name=dinov3_compare
#SBATCH --output=logs/dinov3_compare_%j.out
#SBATCH --error=logs/dinov3_compare_%j.err
...
OUTPUT_DIR="dinov3_comparison"
DEVICE="cuda"

# Model selection (leave empty for all models, or specify subset)
# Example: MODELS="vits16 vitb16 vitl16"
MODELS=""

echo "=========================================="
echo "DINOv3 Model Comparison (SLURM Job)"
echo "=========================================="
...
if [ -n "$MODELS" ]; then
    ...
    echo "Models to compare: ALL (12 models)"
fi
...
# Build command
CMD="python compare_dinov3_models.py \
    --demo-file \"$DEMO_FILE\" \
    --demo-idx $DEMO_IDX \
    --timesteps $TIMESTEPS \
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "  MODELS=\"vits16 vitb16 vitl16\""
else
...
echo ""
echo "Log files:"
echo "  Output: logs/dinov3_compare_${SLURM_JOB_ID}.out"
echo "  Errors: logs/dinov3_compare_${SLURM_JOB_ID}.err"
